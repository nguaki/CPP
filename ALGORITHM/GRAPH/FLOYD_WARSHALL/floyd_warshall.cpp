...
void
printExactPaths(int iaPath[][V], int iSrc, int iDest)
{
   stack<int> S;
   int iFrom;
   ...
   while(1)
   {
        if(iFrom == -1 ){//Check path exists.
            cout << "There is no path\n";
            return;
        }
        S.push(iFrom);
        
        if( iSrc == iFrom ) break; //Check if from vertex and source vertex are the same. Is so, touchdown!!!
...
int main()
{
    ...
    int graph2[V][V] = { {0,   1,  INF, INF},
                         {INF, 0,  -2, INF},
                         {INF, INF, 0,   2},
                         {INF, -6, INF, 0}
                       };
    floydWarshall(graph2, iaPath);
